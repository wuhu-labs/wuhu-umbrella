...
- **Ephemeral O(1) state**: current LLM stream, retry state, bash progress.
  Doesn't need persistence at all.
...
2. **A tail observer** watches the state, diffs cheaply, and writes to SQLite
   asynchronously.
3. **Consumers read the persisted version.** The app, API, and SSE clients observe
   the database, getting a consistent crash-safe view without coupling to the live
   actor.
...
- **SwiftUI projection**: mechanical copy from actor state to `@Observable` class
  for UI binding. Uses `map` + `removeDuplicates` — good enough, optimize later.
- **Single-session in-app mode**: hooks the actor directly without the persistence
  observer. Same actor, different consumers. Enables testable, previewable sessions
  without SQLite.
...
- The **Runner** protocol and runner/mount architecture is sound. Runners talk about
  paths and machines; higher-level resolution happens on the server.
- The **RunnerRegistry**, **RunnerCallbacks** (push-based bash), and the wire
  protocol are clean.
- Contract types in `WuhuCoreClient` and `WuhuCore` are preserved.

## What Goes

- `EffectLoop` and the priority ladder in `AgentBehavior`.
- Persist-first state management.
- The `FileIO` dependency being passed as explicit parameters alongside
  `@Dependency` usage (pick one — `@Dependency` only).
- Mount as a gate for bash execution.
- Materialization conflated with mount.
